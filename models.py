...
class Cliente(db.Model):
    ...
    email = db.Column(db.String(120), unique=True)  # Não mais nullable=False
    nif = db.Column(db.String(20), unique=True)  # Não mais nullable=False
    telefone = db.Column(db.String(20), nullable=False)
    morada = db.Column(db.String(200))  # Não mais nullable=False
...
class Assistencia(db.Model):
    ...
    imei = db.Column(db.String(20))  # Não mais nullable=False
    codigo_seguranca = db.Column(db.String(50), server_default='')
    avaria = db.Column(db.String(200), nullable=False)
    observacoes = db.Column(db.Text)
    tecnico = db.Column(db.String(100), nullable=False)
    valor = db.Column(db.Float, default=0)  # Valor padrão 0
    estado = db.Column(
        Enum('orçamentado', 'reparado', 'entregue', name='estado_assistencia'),
        nullable=False,
